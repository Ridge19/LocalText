...
function createBatch($userId = null)
{
    $batchId         = gs()->batch_id_prefix . '-' . ((Batch::max('id') ?? 0) + 1);
    $batch           = new Batch();
    $batch->user_id  = $userId ?? auth()->id();
...
function exportData($modelName, array $columns = [], int $exportItem = 10, $orderBy = 'ASC', $data = [])
{
    $model                = 'App\\Models\\' . ucfirst($modelName);
    $model                = new $model();
    $model->exportColumns = $columns;
    $model->fileName      = $modelName . '.csv';
    $model->exportItem    = $exportItem;
    $model->data          = $data;
...
function generateInitials($name)
{
    ...
    $initials .= Str::upper(Str::substr($names[0], 0, 1));
    if (isset($names[1])) {
        $initials .= Str::upper(Str::substr($names[1], 0, 1));
    }
...
function apkUrl()
{
    return asset('assets/files/apk/app.apk');
}
