...
class SiteController extends Controller
{
    ...
    public function blog()
    {
        ...
        $seoContents = $sections->seo_content;
        $seoImage = @$seoContents->image ? getImage(getFilePath('seo') . '/' . @$seoContents->image, getFileSize('seo')) : null;
        return view('Template::blogs', compact('pageTitle', 'sections', 'seoContents', 'seoImage'));
    }
    ...
    public function blogDetails($slug)
    {
        ...
        $latestBlogs = Frontend::where('data_keys', 'blog.element')->whereNot('id', $blog->id)->orderBy('id', 'desc')->limit(5)->get();
    ...
    public function placeholderImage($size = null)
    {
        if (!$size || !preg_match('/^\d+x\d+$/', $size)) {
            abort(400, 'Invalid size format. Use WIDTHxHEIGHT like 300x300.');
        }

        [$imgWidth, $imgHeight] = explode('x', $size);
        ...
        $fontSize = max(9, min($fontSize, ($imgHeight < 100 ? 30 : $fontSize)));
