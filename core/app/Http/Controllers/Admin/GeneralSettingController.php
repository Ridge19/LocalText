...
class GeneralSettingController extends Controller
{
    ...
    public function apk()
    {
        ...
        return view('admin.setting.apk_upload', compact('pageTitle'));
    }

    public function uploadApk(Request $request)
    {
        $request->validate([
            'file' => ['required', new FileTypeValidate(['apk'])]
        ]);

        if ($request->hasFile('file')) {
            try {
                fileUploader($request->file, 'assets/files/apk', old: 'app.apk', filename: 'app.apk');
            } catch (\Exception $e) {
                $notify[] = ['error', 'File couldn\'t uploaded'];
                return back()->withNotify($notify);
            }
        }

        $notify[] = ['success', 'APK uploaded successfully'];
        return back()->withNotify($notify);
    }
